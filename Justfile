...
ansible-doctor:
    fish -c 'source ~/venv/ansible-posit/bin/activate.fish; ansible-doctor --version; ansible-doctor roles -r -f; prettier -w .'
...
molecule scenario:
    source ~/venv/ansible-posit/bin/activate.fish && cd extensions && molecule test --scenario-name {{scenario}}
